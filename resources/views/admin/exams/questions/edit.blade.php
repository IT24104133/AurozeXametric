@extends('layouts.dashboard')

@section('title', 'Edit Question')

@section('sidebar-nav')
    @include('admin.partials.sidebar-nav')
@endsection

@section('mobile-nav')
    @include('admin.partials.sidebar-nav')
@endsection

@section('content')
@php
  // Ensure options are ordered (A..E)
  $opts = $question->options ? $question->options->sortBy('position')->values() : collect([]);
  $optCount = $opts->count() > 0 ? $opts->count() : 4;

  // Find correct option index (0..4)
  $correctIndex = 0;
  foreach ($opts as $i => $o) {
    if ((bool)$o->is_correct) { $correctIndex = $i; break; }
  }

  // For old() fallback
  $oldCorrect = old('correct_index');
  if ($oldCorrect !== null && $oldCorrect !== '') {
    $correctIndex = (int)$oldCorrect;
  }

  $keys = ['A','B','C','D','E'];
@endphp

<div class="max-w-3xl mx-auto">
  <div class="flex items-center justify-between mb-6">
    <h1 class="text-2xl font-bold text-slate-900">Edit Question {{ $question->order_index }} – {{ $exam->title }}</h1>
    <a href="{{ route('admin.exams.questions.index', $exam) }}" class="inline-flex items-center gap-2 px-4 py-2 border border-slate-300 rounded-2xl text-slate-700 font-bold hover:bg-slate-50 transition">
      <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/>
      </svg>
      Back to Questions
    </a>
  </div>

  @if ($errors->any())
    <div class="mb-4 p-3 bg-red-50 text-red-700 rounded border border-red-200">
      <div class="font-semibold mb-2">Please fix the errors:</div>
      <ul class="list-disc pl-5 text-sm space-y-1">
        @foreach($errors->all() as $e)
          <li>{{ $e }}</li>
        @endforeach
      </ul>
    </div>
  @endif

  <form method="POST"
        action="{{ route('admin.exams.questions.update', [$exam, $question]) }}"
        class="space-y-4"
        enctype="multipart/form-data">
    @csrf
    @method('PUT')

    {{-- Question Text --}}
    <div>
      <label class="block font-semibold mb-1">Question</label>
      <textarea name="question_text"
                class="w-full border p-2 rounded"
                rows="4"
                required>{{ old('question_text', $question->question_text) }}</textarea>
      @error('question_text') <p class="text-red-600 text-sm">{{ $message }}</p> @enderror
    </div>

    {{-- ✅ Optional Question Images + Previews --}}
    <div class="border rounded p-4 bg-gray-50">
      <h3 class="font-semibold mb-3">Question Images (Optional)</h3>

      <div class="grid grid-cols-1 gap-4">
        <div>
          <label class="block font-semibold mb-1">Image 1 (optional)</label>
          <input type="file" name="image_1" accept="image/*" class="w-full border p-2 rounded bg-white">
          @error('image_1') <p class="text-red-600 text-sm">{{ $message }}</p> @enderror

          @if(!empty($question->image_1))
            <div class="mt-2">
              <p class="text-xs text-gray-600 mb-1">Current Image 1:</p>
              <img src="{{ asset('storage/'.$question->image_1) }}" class="h-28 rounded border bg-white object-contain">
            </div>
          @endif
        </div>

        <div>
          <label class="block font-semibold mb-1">Image 2 (optional)</label>
          <input type="file" name="image_2" accept="image/*" class="w-full border p-2 rounded bg-white">
          @error('image_2') <p class="text-red-600 text-sm">{{ $message }}</p> @enderror

          @if(!empty($question->image_2))
            <div class="mt-2">
              <p class="text-xs text-gray-600 mb-1">Current Image 2:</p>
              <img src="{{ asset('storage/'.$question->image_2) }}" class="h-28 rounded border bg-white object-contain">
            </div>
          @endif
        </div>

        <div>
          <label class="block font-semibold mb-1">Image 3 (optional)</label>
          <input type="file" name="image_3" accept="image/*" class="w-full border p-2 rounded bg-white">
          @error('image_3') <p class="text-red-600 text-sm">{{ $message }}</p> @enderror

          @if(!empty($question->image_3))
            <div class="mt-2">
              <p class="text-xs text-gray-600 mb-1">Current Image 3:</p>
              <img src="{{ asset('storage/'.$question->image_3) }}" class="h-28 rounded border bg-white object-contain">
            </div>
          @endif
        </div>
      </div>

      <p class="text-xs text-gray-600 mt-3">
        Uploading a new file will replace the old image for that slot.
      </p>
    </div>

    {{-- ✅ Options Section --}}
    <div class="border rounded p-4">
      <div class="flex items-center justify-between gap-4 flex-wrap">
        <div>
          <label class="block font-semibold mb-1">Number of Options</label>
          <select id="optionCount" class="border p-2 rounded w-48">
            <option value="4" @selected($optCount === 4)>4 Options</option>
            <option value="5" @selected($optCount === 5)>5 Options</option>
          </select>
        </div>

        <div class="text-sm text-gray-600">
          Choose the correct answer using the “Correct” radio.
        </div>
      </div>

      {{-- correct_index hidden --}}
      <input type="hidden" name="correct_index" id="correct_index" value="{{ $correctIndex }}">

      <div id="optionsWrap" class="mt-4 space-y-3"></div>

      <p class="text-xs text-gray-500 mt-3">
        Each option must have text or an image (or both). Uploading a new option image will replace the old one.
      </p>
    </div>

    <button class="bg-black text-white px-4 py-2 rounded">Update</button>
  </form>
</div>

<script>
(function(){
  const keys = ['A','B','C','D','E'];

  // Existing options from DB (text + image url)
  const existing = @json(
    ($question->options ?? collect([]))
      ->sortBy('position')
      ->values()
      ->map(function($o){
        return [
          'option_text' => $o->option_text,
          'option_image' => $o->option_image ? asset('storage/'.$o->option_image) : null,
        ];
      })
  );

  const optionCountEl = document.getElementById('optionCount');
  const wrap = document.getElementById('optionsWrap');
  const correctIndexEl = document.getElementById('correct_index');

  let count = parseInt(optionCountEl.value || '4', 10);

  function render(){
    wrap.innerHTML = '';

    let correctIndex = parseInt(correctIndexEl.value || '0', 10);
    if (correctIndex >= count) {
      correctIndex = 0;
      correctIndexEl.value = '0';
    }

    for(let i=0; i<count; i++){
      const row = existing[i] || { option_text:'', option_image:null };

      // old() values if validation fails
      const oldText = @json(old('options')) && @json(old('options'))[i] ? (@json(old('options'))[i].option_text || '') : null;

      const div = document.createElement('div');
      div.className = 'border rounded p-3 bg-gray-50';

      div.innerHTML = `
        <div class="flex items-center justify-between mb-2">
          <div class="font-semibold">Option ${keys[i]}</div>

          <label class="inline-flex items-center gap-2 text-sm">
            <input type="radio" name="correct_radio" value="${i}" ${i===correctIndex ? 'checked' : ''}>
            Correct
          </label>
        </div>

        <div class="grid grid-cols-1 gap-3">
          <div>
            <label class="block text-sm font-semibold mb-1">Option Text (optional)</label>
            <input type="text"
                   name="options[${i}][option_text]"
                   class="w-full border p-2 rounded bg-white"
                   placeholder="Type option text..."
                   value="${(oldText !== null ? escapeHtml(oldText) : escapeHtml(row.option_text || ''))}">
          </div>

          <div>
            <label class="block text-sm font-semibold mb-1">Option Image (optional)</label>
            <input type="file"
                   name="options[${i}][option_image]"
                   accept="image/*"
                   class="w-full border p-2 rounded bg-white">
            ${row.option_image ? `<div class="mt-2">
                <p class="text-xs text-gray-600 mb-1">Current Image:</p>
                <img src="${row.option_image}" class="h-24 rounded border bg-white object-contain">
              </div>` : ''}
          </div>
        </div>
      `;

      wrap.appendChild(div);
    }

    document.querySelectorAll('input[name="correct_radio"]').forEach(r => {
      r.addEventListener('change', (e) => {
        correctIndexEl.value = e.target.value;
      });
    });
  }

  optionCountEl.addEventListener('change', () => {
    count = parseInt(optionCountEl.value, 10);
    render();
  });

  function escapeHtml(str){
    if(str === null || str === undefined) return '';
    return String(str)
      .replaceAll('&','&amp;')
      .replaceAll('<','&lt;')
      .replaceAll('>','&gt;')
      .replaceAll('"','&quot;')
      .replaceAll("'","&#039;");
  }

  render();
})();
</script>
@endsection
