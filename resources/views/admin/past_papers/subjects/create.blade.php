        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li>
                <div class="flex items-center">
                    ...
                    <span class="ml-1 text-gray-500">{{ isset($subject) ? 'Edit' : 'Create' }}</span>
                </div>
            </li>
        </ol>
    </nav>
...
@section('content')
    <div class="max-w-2xl mx-auto">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-3xl font-bold text-slate-900">{{ isset($subject) ? 'Edit Subject' : 'Create Subject' }}</h1>
        ...
        <div class="bg-white shadow-sm rounded-3xl border border-slate-200 p-6">
            ...
            <form method="POST" action="{{ isset($subject) ? route('admin.past_papers.subjects.update', ['stream' => $stream, 'subject' => $subject]) : route('admin.past_papers.subjects.store', ['stream' => $stream]) }}">
                @csrf
                @if(isset($subject))
                    @method('PUT')
                @endif
